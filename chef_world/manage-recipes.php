...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="manage-recipes.php">Manage Recipes</a>
                    </li>
                </ul>
    ...
    if(isset($_GET['LtoO']) and $_GET['LtoO']=="click") {
        include("../connection.php");
        $chefid=$_SESSION['user_id'];
        $sql = "SELECT recipeid,postdate,pic, recipename FROM recipe_table where chefid='$chefid'ORDER BY postdate DESC;";
        $r = mysqli_query($con, $sql);
        if(mysqli_num_rows($r) > 0){
            while($row = mysqli_fetch_array($r))
            {
                ...
                echo "    </td>";
                echo "    <td>";
                echo "<a class='btn btn-link btn-rounded btn-sm fw-bold' href='chef_delete_recipe.php?recipeid=$recipeid'>Delete</a>";
                echo "    </td>";
                echo "</tr>";
            }
        }  
        ...
        include("../connection.php");
        $chefid=$_SESSION['user_id'];
        $sql = "SELECT recipeid,postdate,pic,preparationtime, recipename,cuisinetype,description,ingredients,steps FROM recipe_table where chefid='$chefid' ORDER BY postdate ASC;";
        $r = mysqli_query($con, $sql);
        if(mysqli_num_rows($r) > 0){
            while($row = mysqli_fetch_array($r))
            {
        ...
        include("../connection.php");
    $chefid=$_SESSION['user_id'];
    $sql = "SELECT recipeid,postdate,pic,preparationtime,recipename,status FROM recipe_table where chefid='$chefid'";
    $r = mysqli_query($con, $sql);
    if(mysqli_num_rows($r) > 0){
        while($row = mysqli_fetch_array($r))
        {
            ...
            echo " </td>";
            echo " <td>";
                echo " <a class='btn btn-link btn-rounded btn-sm fw-bold'
                    href='update_recipe.php?recipeid=$recipeid'>Update</a>";
                echo " </td>";
            echo " <td>";
                echo "<a class='btn btn-link btn-rounded btn-sm fw-bold'
                    href='chef_delete_recipe.php?recipeid=$recipeid'>Delete</a>";
                echo " </td>";
            echo "</tr>";
            }
            }
